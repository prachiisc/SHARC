...
for file_name, segs in recit:
    ...
    # labels = np.genfromtxt(f'{args.labelspath}/labels_{file_name}',dtype=int)[:,1:]

    all_labels = open(f'{args.labelspath}/labels_{file_name}').readlines()
    labels = []
    ovp_ind = []
    overlap_labels = []
    ...
    for i,line in enumerate(all_labels):
        ...
        if len(line.split()) > 2:
            ovp_ind.append(i)
            ovplabel = int(line.split()[2])
            overlap_labels.append(ovplabel)
       
        labels.append(label)
    # bp()
    labels = np.array(labels)
    overlap_labels = np.array(overlap_labels)
    ovp_ind = np.array(ovp_ind)
    ...
    if use_VB:
        ...
        if len(ovp_ind)>0:
            q_init[ovp_ind,overlap_labels] = 1.0

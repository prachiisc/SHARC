...
for file_name, segs in recit:
    ...
    if use_VB:
        ...
        q, sp, L = VB_diarization.VB_diarization(fea, ubmMeans, invSigma, ubmWeights, V, pi=None, gamma=q_init, maxSpeakers=q_init.shape[1], maxIters=max_iters, VtinvSigmaV=VtinvSigmaV,
...
#######################################################################################
  ...
  #######################################################################################
    starts, ends, out_labels  = merge_adjacent_labels(start, end, labelsAHC)
    mkdir_p(out_rttm_dir+'_pic')
    with open(out_rttm_dir+'_pic'+'/'+file_name+'.rttm', 'w') as fp:
      for l, s, e in zip(out_labels, starts, ends):
        fp.write("SPEAKER %s 1 %.3f %.3f <NA> <NA> %d <NA> <NA>\n" % (file_name, s, e-s, l+1))

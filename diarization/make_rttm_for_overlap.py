...
sys.path.append(os.path.join(os.getcwd(),'steps/libs'))
sys.path.append(os.path.join('steps/libs'))
...
def main():
  ...
  #     num_labels = len(labels)
  #     num_next_labels = len(next_labels)
  #     # if num_labels == num_next_labels == 1:
  #     #   if float(end) == float(next_start) and labels[0] == next_labels:
  #     #     segs[i+1] = ','.join([start, next_end, next_labels])
  #     #   else:
  #     #     new_segs += " " + start + "," + end + "," + labels
  
  #     for count in range(num_labels):
  #       for count2 in range(num_next_labels):
  #         if float(end) == float(next_start) and labels[count] == next_labels[count2]:
  #           segs[i+1] = ','.join([start, next_end, next_labels[count2]])

  #         else:
  #           new_segs += " " + start + "," + end + "," + labels[count]
  ...
  with common_lib.smart_open(args.rttm_file, 'w') as rttm_writer:
    for reco in merged_segs:
      # print('recording: ',reco)
      new_segs = merged_segs[reco]
      for lab in new_segs:
        segs = new_segs[lab].strip().split()
        for i in range(len(segs)):
          start, end, label = segs[i].strip().split(',')         
          print("SPEAKER {0} {1} {2:7.3f} {3:7.3f} <NA> <NA> {4} <NA> <NA>".format(
            reco, args.rttm_channel, float(start), float(end)-float(start), lab), file=rttm_writer)

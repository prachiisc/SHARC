...
def tree_generation(ng):
    ...
    ng.ndata['keep_eid'] = torch.zeros(ng.number_of_nodes()).long().to(ng.device) - 1
    ...
    node_order = tuple(nodes.to(ng.device) for nodes in node_order)
...
def peak_propogation(treeg):
    treeg.ndata['pred_labels'] = torch.zeros(treeg.number_of_nodes()).long().to(treeg.device) - 1
    peaks = torch.where(treeg.in_degrees() == 0)[0].cpu().numpy()
    treeg.ndata['pred_labels'][peaks] = torch.arange(peaks.shape[0]).to(treeg.device)
    def message_func(edges):
    ...
    node_order = tuple(nodes.to(treeg.device) for nodes in node_order)
...
def decode(g, tau, threshold, use_gt, mode,
           ids=None, global_edges=None, global_num_nodes=None, global_peaks=None,device='cpu'):
    ...
    if "train" in mode:
        g.apply_edges(lambda edges: {'keep': (edges.src[den_key] > edges.dst[den_key]).long() * \
                                         ...
                                         (edges.src['labels'] == edges.dst['labels']).long()})
    elif "test" in mode:
        g.apply_edges(lambda edges: {'keep': (edges.src[den_key] > edges.dst[den_key]).long() * \
                                         (edges.data['edge_dist'] < 1 - tau).long()})  
    ...
    # after this some nodes are left with no edges. Therefore the tree generation throws error that some node 
    # does not contain valid edges.
   
    # Tree generation
    ng.edata[dgl.EID] = torch.arange(ng.number_of_edges()).to(ng.device)
    ...
    src, dst = treeg.edges()
    new_global_edges = (global_edges[0] + ids[src.cpu().numpy()].tolist(), 
                        global_edges[1] + ids[dst.cpu().numpy()].tolist()) 
  
    global_treeg = dgl.graph(new_global_edges, num_nodes=global_num_nodes).to(ng.device)
...
def build_next_level_e2e(mfcc_xvec_features,features, labels, peaks,
                     global_features, global_pred_labels, global_peaks):
    global_peak_to_label = global_pred_labels[global_peaks]
    global_label_to_peak = np.zeros_like(global_peak_to_label)
    for i, pl in enumerate(global_peak_to_label):
    ...
    cluster_ind = np.split(np.argsort(global_pred_labels),
                           np.unique(np.sort(global_pred_labels), return_index=True)[1][1:])
    cluster_mfcc_xvec_features = torch.zeros((len(peaks), global_features.shape[1])).to(global_features.device)
  
    for pi in range(len(peaks)):
        cluster_mfcc_xvec_features[global_label_to_peak[pi],:] = torch.mean(global_features[cluster_ind[pi],:], dim=0)
    mfcc_xvec_features = mfcc_xvec_features[peaks]
    features = features[peaks]
    labels = labels[peaks]
    return mfcc_xvec_features, features, labels, cluster_mfcc_xvec_features
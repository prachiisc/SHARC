...
if [ $stage == "AMI_esharc_plda" ];then
    # log.2.tau0.0_k50.txt
    ...
    xvecpath=exp_sharc/results_with_ami_sdm_train_e2e_fulltrain_sharcinitk60_lr0.001_k30/withnonorm/${dataset}/esharc_xvectors_20/xvector.scp
    labelspath=tools_diar/ALL_GROUND_LABELS/${dataset}/threshold_0.5/
    pldamodel=lists_esharc/${trainset}_gnd/plda_${trainset}_gnd.pkl
    ...
    for epochs in 20 40 60 80;do
    dropout=0.0

    # model_filename=checkpoint_pre_trained/ami/sharc/ami_sdm_train_model_best.pth
    # out_path=exp_sharc/results_with_${trainset}/${dataset}/labels_withoutglobalfeats_norm_full_${epochs}_lr${lr}_traink${traink}_proc/dropout${dropout}
    model_filename=checkpoint/${trainset}/pretrain_sharcagain_withesharc_xvecs/${trainset}_nonoverlap_sampler_6_PLDA_lr${lr}_dropout${dropout}/model_${epochs}_snapshot.pth
    out_path=exp_sharc/results_with_${trainset}_esharc_plda/${dataset}/labels_withoutglobalfeats_norm_full_${epochs}_lr${lr}_traink${traink}_proc/dropout${dropout}
